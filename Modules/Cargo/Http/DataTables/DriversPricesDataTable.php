...
class DriversPricesDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('select', function (DriversPrice $model) {
                $adminTheme = env('ADMIN_THEME', 'adminLte');
                return view($adminTheme.'.components.modules.datatable.columns.checkbox', ['model' => $model, 'ifHide' => $model->id == 0]);
            })
            ->editColumn('id', function (DriversPrice $model) {
                return '#'.$model->id;
            })
            ->editColumn('name', function (DriversPrice $model) {
                return $model->name;
            })
          
            ->editColumn('created_at', function (DriversPrice $model) {
                return date('d M, Y H:i', strtotime($model->created_at));
            })
            ->addColumn('action', function (DriversPrice $model) {
                $adminTheme = env('ADMIN_THEME', 'adminLte');return view('cargo::'.$adminTheme.'.pages.drivers_prices.columns.actions', ['model' => $model, 'table_id' => $this->table_id]);
            });
    }
     ...
     * @return Area
     */
    public function query(DriversPrice $model, Request $request)
    {
